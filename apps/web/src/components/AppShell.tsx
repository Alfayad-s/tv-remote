...
export function AppShell({
  ...
  if (immersive) {
    return (
      <div className="flex h-dvh max-h-dvh w-full flex-col bg-paper pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
        {inner}
      </div>
    );
  }
  ...
  return (
    <div className="flex h-dvh max-h-dvh w-full justify-center px-4 pt-[max(1rem,env(safe-area-inset-top))] pb-[max(1rem,env(safe-area-inset-bottom))]">
      <div className="flex min-h-0 w-full max-w-[430px] flex-col overflow-hidden border-[5px] border-ink bg-paper shadow-[8px_8px_0_#111]">
